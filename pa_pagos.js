...
function loadButtons() {
    ...
    $(".ewTableRow , .ewTableAltRow").each(function (index) {
        ...
        if ($.inArray("" + tempId, IDs) != -1) {
            tempBut = "<i id='butPag" + tempId + "' title='Pagar' class='butPago desactivado fa fa-money fa-2x'></i>";
        } else {
            tempBut = "<i id='butPag" + tempId + "' title='Pagar' class='butPago fa fa-money fa-2x' onclick='preguntarPago(" + tempId + ",\"" + tipotemp + "\",\"" + consectemp + "\"," + montotemp + ")'></i>";
        }
...
function adicionarFactura(id, tipo, consec, monto) {
    ...
    htmlTemp = '<tr id="factutemp' + id + '"><td>&nbsp;' + tipo + '</td><td>&nbsp;' + consec + '</td><td>&nbsp;' + Globalize.format(parseInt(monto), "c") + '</td><td><i title="Remover documento" alt="Remover documento" onclick="removerDocumento(' + id + ')" class="fa fa-times" onlick="removerDocumento(' + id + ')"></i></td></tr>';
...
function initCookies() {
    ...
    if ($.isNumeric($.cookie("numFacturas")) && parseInt($.cookie("numFacturas")) > 0) {
        ...
        for (t = 0; t < IDs.length; t++) {
            ...
            htmlTemp = '<tr id="factutemp' + tempFact[0] + '"><td>&nbsp;' + tempFact[1] + '</td><td>&nbsp;' + tempFact[2] + '</td><td>&nbsp;' + Globalize.format(parseInt(tempFact[3]), "c") + '</td><td><i title="Remover documento" alt="Remover documento" onclick="removerDocumento(' + tempFact[0] + ')" class="fa fa-times" onlick="removerDocumento(' + tempFact[0] + ')"></i></td></tr>';
            $("#facturasPagar").append(htmlTemp);
            $("#tabs").tabs("option", "disabled", []);
        }
    } else {
