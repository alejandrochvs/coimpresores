...
if (!empty($arlast)){
	...
	for($f=0; $f<sizeof($arDoctos); $f++){
		...
		if($f>0)
			$descripcion .= ", ";
		$descripcion .= trim($arDoctos[$f]["tipo_docto"]).trim($arDoctos[$f]["consec_docto"])."($".number_format($montopago, 0).")";
	}
}
...
<tr bgcolor="#F1F1F1">
  ...
  <td><p>
    ...
    <p>
     ...
     <?php /*?> <input name="prueba" type="hidden" value="1" /><?php */?>
      <input name="url_respuesta" type="hidden" value="http://www.coimpresores.com.co/pagos/pol_respuesta.php" />
      <input name="url_confirmacion" type="hidden" value="http://www.coimpresores.com.co/pagos/pol_confirmacion.php" />
