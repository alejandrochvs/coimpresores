...
$descripcion = "Pago de los siguientes documentos: ";
...
$codigo_respuesta_pol = array(
    ...
    3 => "Pago cancelado por el usuario durante validación Transacción rechazada por la entidad",
...
$sqlhistorial = "SELECT `historial_pagos`.idhistorial_pagos, `historial_pagos`.ref_venta, `historial_pagos`.monto_pago AS pago_total, pagos_x_docto.iddoctocontable, pagos_x_docto.tipo_docto, pagos_x_docto.consec_docto, pagos_x_docto.valor, pagos_x_docto.monto_pago FROM `historial_pagos` JOIN pagos_x_docto ON ( pagos_x_docto.historial = historial_pagos.idhistorial_pagos ) WHERE historial_pagos.ref_venta LIKE '$refVenta'";
	...
	for($t=0; $t< sizeof($arHistorial); $t++){
		...
		if($t>0)
			$documentos .= "<br>";
		$documentos .= $arHistorial[$t]["tipo_docto"]." ".$arHistorial[$t]["consec_docto"] . " ($ ".number_format($arHistorial[$t]["monto_pago"], 0).")";
	}
...
<tr bgcolor="#F1F1F1">
  <td>Respuesta:</td>
  <td><p>
<?php echo $codigo_respuesta_pol[$_GET["codigo_respuesta_pol"]] ?>
      
      </p></td> 
</tr>
<tr bgcolor="#F1F1F1">
  <td>Descripcion Respuesta:</td>
  <td><p>
...
<tr bgcolor="#F1F1F1">
  ...
  <td><p>
<?php echo "$ ".number_format($_GET["valor"], 0) ?>
...
<tr bgcolor="#F1F1F1">
  <td>Fecha:</td>
  <td><p>
<?php echo $_GET["fecha_procesamiento"]?>
