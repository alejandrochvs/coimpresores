...
userpriv.FormID = "fnivelesuserpriv"; // form ID
...
<p class="phpmaker"><a href="niveleslist.php"><?php echo $Language->Phrase("BackToList") ?></a></p>
<p class="phpmaker"><?php echo $Language->Phrase("UserLevel") ?><?php echo $Security->GetUserLevelName($niveles->idnivel->CurrentValue) ?>(<?php echo $niveles->idnivel->CurrentValue ?>)</p>
<?php
...
for ($i = 0; $i < $userpriv->TableNameCount; $i++) {
	$userpriv->TempPriv = $Security->GetUserLevelPrivEx($EW_USER_LEVEL_TABLE_NAME[$i], $niveles->idnivel->CurrentValue);
...
class cuserpriv {
	...
	function cuserpriv() {
		...
		if (!isset($GLOBALS["niveles"])) $GLOBALS["niveles"] = new cniveles();
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel('niveles');
	...
	function Page_Main() {
		...
		// Get action
		if (@$_POST["a_edit"] == "") {
			$niveles->CurrentAction = "I"; // Display with input box
			...
			if ($niveles->idnivel->QueryStringValue == "-1") {
				$this->Disabled = " disabled=\"disabled\"";
			} else {
				$this->Disabled = "";
			}
		} else {
			$niveles->CurrentAction = $_POST["a_edit"];

			// Get fields from form
			$niveles->idnivel->setFormValue($_POST["x_idnivel"]);
	...
	// Update privileges
	function EditRow() {
		global $conn, $EW_USER_LEVEL_TABLE_NAME, $niveles;
		for ($i = 0; $i < $this->TableNameCount; $i++) {
			$Sql = "SELECT * FROM " . EW_USER_LEVEL_PRIV_TABLE . " WHERE " . 
				...
				EW_USER_LEVEL_PRIV_USER_LEVEL_ID_FIELD . " = " . $niveles->idnivel->CurrentValue;
			$rs = $conn->Execute($Sql);
			if ($rs && !$rs->EOF) {
				...
				$Sql = "INSERT INTO " . EW_USER_LEVEL_PRIV_TABLE . " (" . EW_USER_LEVEL_PRIV_TABLE_NAME_FIELD . ", " . EW_USER_LEVEL_PRIV_USER_LEVEL_ID_FIELD . ", " . EW_USER_LEVEL_PRIV_PRIV_FIELD . ") VALUES ('" . ew_AdjustSql($EW_USER_LEVEL_TABLE_NAME[$i]) . "', " . $niveles->idnivel->CurrentValue . ", " . $this->Privileges[$i] . ")";
				$conn->Execute($Sql);
			}
			if ($rs)
				$rs->Close();
