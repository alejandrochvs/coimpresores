...
class cauditoria {
	...
	function cauditoria() {
		...
		// fecha_hora
		$this->fecha_hora = new cField('auditoria', 'auditoria', 'x_fecha_hora', 'fecha_hora', '`fecha_hora`', 135, 11, FALSE, '`fecha_hora`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fecha_hora->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		...
		// campo
		$this->campo = new cField('auditoria', 'auditoria', 'x_campo', 'campo', '`campo`', 200, -1, FALSE, '`campo`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['campo'] =& $this->campo;

		// valorclave
		$this->valorclave = new cField('auditoria', 'auditoria', 'x_valorclave', 'valorclave', '`valorclave`', 201, -1, FALSE, '`valorclave`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['valorclave'] =& $this->valorclave;

		// viejovalor
		$this->viejovalor = new cField('auditoria', 'auditoria', 'x_viejovalor', 'viejovalor', '`viejovalor`', 201, -1, FALSE, '`viejovalor`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['viejovalor'] =& $this->viejovalor;

		// nuevovalor
		$this->nuevovalor = new cField('auditoria', 'auditoria', 'x_nuevovalor', 'nuevovalor', '`nuevovalor`', 201, -1, FALSE, '`nuevovalor`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['nuevovalor'] =& $this->nuevovalor;
	}
...
//			$AddUrl .= "?" . $sUrlParm;
	...
	function LoadListRowValues(&$rs) {
		...
		$this->campo->setDbValue($rs->fields('campo'));
		$this->valorclave->setDbValue($rs->fields('valorclave'));
		$this->viejovalor->setDbValue($rs->fields('viejovalor'));
		$this->nuevovalor->setDbValue($rs->fields('nuevovalor'));
	}
   ...
   // Common render codes
		...
		$this->fecha_hora->ViewValue = ew_FormatDateTime($this->fecha_hora->ViewValue, 11);
		...
		// campo
		$this->campo->ViewValue = $this->campo->CurrentValue;
		$this->campo->ViewCustomAttributes = "";

		// valorclave
		$this->valorclave->ViewValue = $this->valorclave->CurrentValue;
		$this->valorclave->ViewCustomAttributes = "";

		// viejovalor
		$this->viejovalor->ViewValue = $this->viejovalor->CurrentValue;
		$this->viejovalor->ViewCustomAttributes = "";

		// nuevovalor
		$this->nuevovalor->ViewValue = $this->nuevovalor->CurrentValue;
		$this->nuevovalor->ViewCustomAttributes = "";
		...
		// campo
		$this->campo->LinkCustomAttributes = "";
		$this->campo->HrefValue = "";
		$this->campo->TooltipValue = "";

		// valorclave
		$this->valorclave->LinkCustomAttributes = "";
		$this->valorclave->HrefValue = "";
		$this->valorclave->TooltipValue = "";

		// viejovalor
		$this->viejovalor->LinkCustomAttributes = "";
		$this->viejovalor->HrefValue = "";
		$this->viejovalor->TooltipValue = "";

		// nuevovalor
		$this->nuevovalor->LinkCustomAttributes = "";
		$this->nuevovalor->HrefValue = "";
		$this->nuevovalor->TooltipValue = "";
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					$XmlDoc->AddField('fecha_hora', $this->fecha_hora->ExportValue($this->Export, $this->ExportOriginalValue));
					...
					$XmlDoc->AddField('campo', $this->campo->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('valorclave', $this->valorclave->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('viejovalor', $this->viejovalor->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('nuevovalor', $this->nuevovalor->ExportValue($this->Export, $this->ExportOriginalValue));
				} else {
					$XmlDoc->AddField('id', $this->id->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('fecha_hora', $this->fecha_hora->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('script', $this->script->ExportValue($this->Export, $this->ExportOriginalValue));
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				$Doc->ExportCaption($this->campo);
				$Doc->ExportCaption($this->valorclave);
				$Doc->ExportCaption($this->viejovalor);
				$Doc->ExportCaption($this->nuevovalor);
			} else {
				$Doc->ExportCaption($this->id);
				$Doc->ExportCaption($this->fecha_hora);
				$Doc->ExportCaption($this->script);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					$Doc->ExportField($this->campo);
					$Doc->ExportField($this->valorclave);
					$Doc->ExportField($this->viejovalor);
					$Doc->ExportField($this->nuevovalor);
				} else {
					$Doc->ExportField($this->id);
					$Doc->ExportField($this->fecha_hora);
					$Doc->ExportField($this->script);
